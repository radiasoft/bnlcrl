...
class DeltaFinder:
    def __init__(self, **kwargs):
        ...
        self.default_e_min = self.parameters["e_min"]["type"](
            self.parameters["e_min"]["default"]
        )
        self.default_e_max = self.parameters["e_max"]["type"](
            self.parameters["e_max"]["default"]
        )
        ...
        if self.calc_delta:
            if self.available_libs["periodictable"]:
                ...
                raise ValueError(
                    '"periodictable" library is not available. Install it if you want to use it.'
                )
        else:
        ...
        if self.save_output:
            ...
            file_name = "{}.json".format(
                _output_file_name(self.elements, self.characteristic)
            )
    ...
    def _find_characteristic_value(self):
        ...
        if self.use_numpy and self.available_libs["numpy"]:
            if self.data_file:
                ...
                try:
                    idx_previous = self.numpy.where(
                        data[:, energy_column] <= self.energy
                    )[0][-1]
                    idx_next = self.numpy.where(data[:, energy_column] > self.energy)[
                        0
                    ][0]
                except IndexError:
                    raise Exception(
                        error_msg.format(self.default_e_min, self.default_e_max)
                    )

                idx = (
                    idx_previous
                    if abs(data[idx_previous, energy_column] - self.energy)
                    <= abs(data[idx_next, energy_column] - self.energy)
                    else idx_next
                )
                ...
                raise Exception(
                    "Processing with NumPy is only possible with the specified file, not content."
                )
        else:
            if not self.content:
            ...
            for i in range(skiprows, len(self.content)):
                ...
                characteristic_values.append(
                    float(self.content[i].split()[characteristic_value_column])
                )
            ...
            idx = (
                idx_previous
                if abs(energies[idx_previous] - self.energy)
                <= abs(energies[idx_next] - self.energy)
                else idx_next
            )
    ...
    def _get_remote_file_name(self, formula=None):
        ...
        payload = {
            ...
            self.server_info[self.characteristic]["fields"][
                "material"
            ]: "Enter Formula",
        ...
        if self.characteristic == "atten":
            ...
            payload[self.server_info[self.characteristic]["fields"]["output"]] = (
                "Plot",
            )
            ...
            payload[self.server_info[self.characteristic]["fields"]["thickness"]] = (
                self.thickness
            )  # um
        r = self.requests.post(
            "{}{}".format(
                self.server_info["server"],
                self.server_info[self.characteristic]["post_url"],
            ),
            payload,
        )
        content = r.text
        ...
        try:
            self.file_name = str(
                content.split(
                    "{}=".format(self.server_info[self.characteristic]["file_tag"])
                )[1]
                .split(">")[0]
                .replace('"', "")
            )
    ...
    def _request_from_server(self):
        if self.available_libs["requests"]:
            d = []
            for f in self.formula.split(
                ","
            ):  # to support multiple chemical elements comma-separated list
...
def _output_file_name(elements, characteristic):
    return (
        "{}_{}".format(",".join(elements), characteristic)
        if len(elements) > 1
        else characteristic
    )
